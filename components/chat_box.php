<?php
// Use the variables passed from the parent page
    ...
    <div class="<?php echo $header_bg; ?> p-4 flex justify-between items-center text-white shadow-md z-10">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center font-bold text-white">
                <?= ($chat_user_type === 'Professional') ? 'PRO' : 'PT' ?>
            </div>
            <div>
        ...
        <span class="text-xs bg-black/20 px-2 py-1 rounded"><?php echo $chat_user_type; ?> View</span>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // CHANGED: Use the PHP variable for myType
        const myType = <?php echo json_encode($chat_user_type); ?>;
        ...
        const render = (messages) => {
            if (!messages || messages.length === 0) {
                container.innerHTML = '<div class="text-center text-gray-500 text-sm mt-4">No messages yet.</div>';
                return;
            }
            ...
            const html = messages.map(msg => {
                ...
                const time = new Date(msg.Chat_Time).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit'
                });
            ...
            try {
                const res = await fetch(`${API_URL}?action=send`, {
                    ...
                    body: JSON.stringify({
                        ...
                        sender_id: <?php echo json_encode($chat_my_id); ?>,
                        receiver_id: <?php echo json_encode($chat_target_id); ?>
                    })
                });
                const result = await res.json();
                if (!result.success) console.error("Send Error:", result.error);
                fetchMessages();
            } catch (err) {
