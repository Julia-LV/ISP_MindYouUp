...
// ---------- PAGE SETUP (used by header_component.php) ----------
$page_title  = 'Resource Hub Admin';
$body_class  = 'h-full';
$no_layout   = false; // if your header_component supports it
...
function delete_resource(mysqli $conn, int $id, array &$errors, string &$success): void
{
    ...
    foreach (array($media, $image, $thumb) as $stored) {
        if (!empty($stored)) {
            $stored = trim($stored);
            $rel = str_starts_with($stored, 'uploads/') ? $stored : 'uploads/' . ltrim($stored, '/');
            $fsPath = __DIR__ . '/../../' . $rel;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($item_type === 'skill') {
        if ($skill_key === '') {
            ...
            if ($title === '') $title = $skills[$skill_key];
        }
    ...
    if (empty($errors)) {
        ...
        if ($stmtFind = $conn->prepare($sqlFind)) {
            $stmtFind->bind_param('isss', $currentProfessionalId, $item_type, $title, $skill_key);
            $stmtFind->execute();
            $resFind = $stmtFind->get_result();
            if ($rowF = $resFind->fetch_assoc()) $existingId = (int)$rowF['id'];
            $stmtFind->close();
        }
        ...
        if ($currentProfessionalId && $newResourceId && !empty($selectedSharePatients) && empty($errors)) {
            ...
            foreach ($selectedSharePatients as $pid) {
                $pid = (int)$pid;
                if ($pid <= 0) continue;
                $ins->bind_param('iiisiii', $pid, $newResourceId, $currentProfessionalId, $skillKeyVar, $pid, $newResourceId, $currentProfessionalId);
                $ins->execute();
            }
            $ins->close();
        }
...
// IMPORTANT: include these BEFORE any HTML output (so redirects above remain safe)
require_once __DIR__ . '/../../components/header_component.php';
require_once __DIR__ . '/../../includes/navbar.php';
?>

<!-- Page content (no second doctype/head/body here) -->
<style>
    ...
    * { box-sizing: border-box; }
    body { margin:0; font-family: system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; background:var(--light-green); color:var(--preto); padding-left:80px; }
    ...
    input[type="text"], textarea, select { width:100%; margin-top:6px; padding:10px 11px; border-radius:12px; border:1px solid #D1D5DB; background:#F9FAFB; font-size:14px; font-family:inherit; }
    textarea { min-height:100px; resize:vertical; }
    input[type="file"] { margin-top:6px; font-size:13px; }
    .hint { font-size:12px; color:#6b7280; margin-top:4px; }
    .btn-primary { padding:11px 24px; border-radius:999px; border:none; cursor:pointer; background:var(--verde); color:#fff; font-size:15px; font-weight:600; box-shadow:0 7px 16px rgba(0,0,0,0.15); }
    .btn-primary:hover { background:#00453F; }
    .btn-secondary-link { display:inline-flex; padding:11px 24px; border-radius:999px; border:1px solid var(--verde); background:#ffffff; color:var(--verde); font-size:14px; font-weight:600; }
    .btn-secondary-link:hover { background:#e6f3ec; }
    .btn-library-link { display:inline-flex; padding:11px 24px; border-radius:999px; border:1px solid var(--coral); background:#FFF7E1; color:#F26647; font-size:14px; font-weight:600; box-shadow:0 7px 16px rgba(0,0,0,0.06); }
    .btn-library-link:hover { background:#FFE9C2; }
    .actions-row { display:flex; gap:15px; align-items:center; margin-top:24px; flex-wrap:wrap; }
    ...
    .share-search input { width:100%; padding:8px 10px; border-radius:999px; border:1px solid #D1D5DB; background:#F9FAFB; font-size:13px; }
    .share-list { max-height:220px; overflow:auto; border-radius:var(--radius-md); border:1px solid #E5E7EB; background:#F9FAFB; padding:6px 8px; }
    ...
    .select-wrapper select { appearance:none; width:100%; margin-top:6px; padding:10px 36px 10px 11px; border-radius:12px; border:1px solid #D1D5DB; background:#F9FAFB; font-size:14px; cursor:pointer; }
    .select-arrow { position:absolute; right:12px; top:50%; transform:translateY(-50%); pointer-events:none; font-size:14px; color:#6b7280; }
...
<!-- If navbar.php needs Tailwind CDN and your header_component doesn't include it, add this ONCE in header_component.php -->
    ...
    <div class="shell-inner">
        ...
        <section class="card">
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="share-header"><span>Share to</span></div>
                        ...
                        <div class="share-list" id="shareList">
                            <?php foreach ($linkedPatients as $p): ?>
                                ...
                                <div class="share-row" data-name="<?php echo htmlspecialchars(strtolower($p['First_Name'] . ' ' . $p['Last_Name'])); ?>">
                                    <span class="share-name"><?php echo htmlspecialchars($p['First_Name'] . ' ' . $p['Last_Name']); ?></span>
                ...
                <div class="actions-row">
                    ...
                    <a href="resourcehub_existing.php" class="btn-secondary-link">View existing resources</a>
                    <a href="resourcehub_library.php" class="btn-library-link">View resource library</a>
                </div>
            </form>
        </section>
    </div>
