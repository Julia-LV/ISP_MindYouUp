    ...
    if (isset($_GET['offset'])) {
        ...
        if ($conn) {
            if ($graph_type === 'main') {
                ...
                $response['stress'] = $stress; $response['severity'] = $severity;
            }
            ...
            elseif ($graph_type === 'multi') {
                ...
                for ($i = 6; $i >= 0; $i--) {
                    ...
                    $stress[] = round($resE['s'] ?? 0, 1); $anxiety[] = round($resE['a'] ?? 0, 1); $sleep[] = round($resE['sl'] ?? 0, 1);
                    ...
                    $intensity[] = $resT['i'] ?? 0; $frequency[] = (int)$resT['c']; 
                }
            ...
            elseif ($graph_type === 'hourly') {
                ...
                $stmt->execute();
                $res = $stmt->get_result();
                while($row = $res->fetch_assoc()) { $hourly[$row['h']] = $row['c']; }
            ...
            elseif ($graph_type === 'moods') {
                ...
                $stmt->execute();
                $res = $stmt->get_result();
                while($row = $res->fetch_assoc()) { $moods[] = ucfirst($row['Emotion']); $counts[] = $row['c']; }
            ...
            elseif ($graph_type === 'muscles') {
                ...
                $stmt->execute();
                $res = $stmt->get_result();
                while($row = $res->fetch_assoc()) { $muscles[] = $row['Muscle_Group']; $counts[] = $row['c']; }
...
if ($conn) {
    ...
    if ($result) {
        while($row = $result->fetch_assoc()) {
            $patients[] = ['id' => $row['User_ID'], 'name' => $row['First_Name'] . ' ' . $row['Last_Name']];
        }
    }
}
...
if ($conn && $selected_patient_id) {
    ...
    $stmt->bind_param("i", $selected_patient_id); $stmt->execute();
    $res = $stmt->get_result(); while($r=$res->fetch_assoc()){$recent_logs[]=$r;} $stmt->close();
    ...
    $stmt->bind_param("i", $selected_patient_id); $stmt->execute();
    $res = $stmt->get_result(); while($r=$res->fetch_assoc()){$recent_emotions[]=$r;} $stmt->close();
}
                        ...
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <?php if (count($recent_logs) > 0): ?>
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <tr><td colspan="3" class="px-6 py-8 text-center text-gray-400">No logs found.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
                        ...
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <?php if (count($recent_emotions) > 0): ?>
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <tr><td colspan="3" class="px-6 py-8 text-center text-gray-400">No entries found.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
...
<div id="modal-tics" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-4xl h-[80vh] flex flex-col">
        <div class="px-6 py-4 border-b flex justify-between items-center"><h3 class="text-lg font-bold text-gray-800">All Tic Logs</h3><button onclick="closeModal('modal-tics')" class="text-gray-500 hover:text-red-500"><i data-lucide="x" class="w-6 h-6"></i></button></div>
        <div class="flex-1 overflow-y-auto p-6"><table class="w-full text-sm text-left"><thead class="bg-gray-50 text-gray-500 font-medium"><tr><th class="p-3">Date</th><th class="p-3">Type</th><th class="p-3">Description</th><th class="p-3">Intensity</th><th class="p-3">Notes</th></tr></thead><tbody id="table-body-tics" class="divide-y divide-gray-100"><tr><td colspan="5" class="p-4 text-center">Loading...</td></tr></tbody></table></div>
    </div>
</div>
<div id="modal-emotions" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-4xl h-[80vh] flex flex-col">
        <div class="px-6 py-4 border-b flex justify-between items-center"><h3 class="text-lg font-bold text-gray-800">All Emotional Entries</h3><button onclick="closeModal('modal-emotions')" class="text-gray-500 hover:text-red-500"><i data-lucide="x" class="w-6 h-6"></i></button></div>
        <div class="flex-1 overflow-y-auto p-6"><table class="w-full text-sm text-left"><thead class="bg-gray-50 text-gray-500 font-medium"><tr><th class="p-3">Date</th><th class="p-3">Emotion</th><th class="p-3">Stress</th><th class="p-3">Anxiety</th><th class="p-3">Sleep</th><th class="p-3">Notes</th></tr></thead><tbody id="table-body-emotions" class="divide-y divide-gray-100"><tr><td colspan="6" class="p-4 text-center">Loading...</td></tr></tbody></table></div>
    </div>
</div>
...
<script>
    ...
    async function fetchAllData(action, tbodyId, fields) {
        ...
        try {
            ...
            tbody.innerHTML = '';
            if (json.data.length === 0) { tbody.innerHTML = `<tr><td colspan="${fields.length}" class="p-4 text-center text-gray-400">No records found.</td></tr>`; return; }
            json.data.forEach(row => {
                let tr = `<tr class="hover:bg-gray-50">`;
                fields.forEach(f => { tr += `<td class="p-3 border-b border-gray-50">${row[f] || '-'}</td>`; });
                tr += `</tr>`;
                tbody.innerHTML += tr;
            });
        } catch (e) { tbody.innerHTML = `<tr><td colspan="${fields.length}" class="p-4 text-center text-red-500">Error loading data.</td></tr>`; }
    }
    ...
    const comboChart = new Chart(ctxCombo, {
        type: 'bar',
        data: { labels: initialLabels, datasets: [ { type: 'line', label: 'Avg Stress', data: initialStress, borderColor: '#fb923c', borderWidth: 2, borderDash: [5, 5], pointRadius: 4, tension: 0.3, yAxisID: 'y' }, { type: 'bar', label: 'Max Intensity', data: initialSeverity, backgroundColor: '#005949', borderRadius: 4, barThickness: 24, yAxisID: 'y' } ] },
    ...
    const chartHourly = new Chart(document.getElementById('chart-hourly'), { type: 'line', data: { labels: [...Array(24).keys()].map(h => h + ":00"), datasets: [] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { grid: {display: false} }, y: { beginAtZero: true } }, plugins: { legend: { display: false } } } });
    const chartMoods = new Chart(document.getElementById('chart-moods'), { type: 'pie', data: { labels: [], datasets: [] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { boxWidth: 10, font: {size: 11} } } } } });
    const chartMuscles = new Chart(document.getElementById('chart-muscles'), { type: 'polarArea', data: { labels: [], datasets: [] }, options: { responsive: true, maintainAspectRatio: false, scales: { r: { ticks: { display: false } } }, plugins: { legend: { position: 'right', labels: { boxWidth: 10 } } } } });
    ...
    function changeOffset(type, delta) { offsets[type] += delta; if (offsets[type] < 0) offsets[type] = 0; loadGraph(type); }
    ['sleep', 'multi', 'hourly', 'moods', 'muscles'].forEach(t => loadGraph(t));
    function exportPDF() { const element = document.getElementById('report-container'); const header = document.getElementById('pdf-header'); header.classList.remove('hidden'); html2pdf().set({ margin: [0.5, 0.5], filename: 'Report.pdf', image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2 }, jsPDF: { unit: 'in', format: 'letter', orientation: 'portrait' } }).from(element).save().then(() => header.classList.add('hidden')); }
</script>