...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if ($step === 1) {
		// Save A1 answers
		$_SESSION['ygtss_answers']['A1'] = $_POST['A1'] ?? [];
		$step = 2;
	} elseif ($step === 2) {
		if (isset($_POST['back'])) {
			$step = 1;
			...
			// Save A2 answers
			$_SESSION['ygtss_answers']['A2'] = $_POST['A2'] ?? [];
			$step = 3;
		}
	}
}
...
// Questions for A2: Tic Severity Scale (full, detailed)
$a2_questions = [
	'1. NUMBER OF TICS' => [
		[
		...
		<form method="post">
			...
			<?php foreach ($a1_questions as $group => $questions): ?>
				...
				<?php foreach ($questions as $qtext): ?>
					<div class="mb-2 ms-3">
						<label><?= htmlspecialchars($qtext) ?></label><br>
						<?php foreach (["Never", "Past", "Present"] as $opt): ?>
						<div class="form-check">
								<input class="form-check-input" type="radio" name="A1[<?= $qnum ?>][answer]" value="<?= $opt ?>" id="A1_<?= $qnum ?>_<?= $opt ?>">
								<label class="form-check-label" for="A1_<?= $qnum ?>_<?= $opt ?>">
									<?= $opt ?>
								</label>
							</div>
						<?php endforeach; ?>
						<input type="text" name="A1[<?= $qnum ?>][onset]" placeholder="Age of onset" class="form-control form-control-sm mt-1" style="max-width:150px;display:inline-block;" />
					</div>
					<?php $qnum++; ?>
				<?php endforeach; ?>
			<?php endforeach; ?>
			...
			<div class="mb-3 mt-4">
				...
				<div class="form-check">
					<input class="form-check-input" type="radio" name="A1_simultaneous_tics" value="Yes" id="A1_simultaneous_tics_yes">
					<label class="form-check-label" for="A1_simultaneous_tics_yes">Yes</label>
				</div>
				<div class="form-check">
					<input class="form-check-input" type="radio" name="A1_simultaneous_tics" value="No" id="A1_simultaneous_tics_no">
					<label class="form-check-label" for="A1_simultaneous_tics_no">No</label>
				</div>
				<input type="text" name="A1_simultaneous_tics_desc" placeholder="If yes, describe" class="form-control form-control-sm mt-2" />
			</div>
			<div class="mb-3">
				<label><strong>Do you have more than one group of tics that occur together?</strong></label><br>
				<div class="form-check">
					<input class="form-check-input" type="radio" name="A1_multiple_groups" value="Yes" id="A1_multiple_groups_yes">
					<label class="form-check-label" for="A1_multiple_groups_yes">Yes</label>
				</div>
				<div class="form-check">
					<input class="form-check-input" type="radio" name="A1_multiple_groups" value="No" id="A1_multiple_groups_no">
					<label class="form-check-label" for="A1_multiple_groups_no">No</label>
				</div>
				<input type="text" name="A1_multiple_groups_desc" placeholder="If yes, describe" class="form-control form-control-sm mt-2" />
			</div>
		...
		<form method="post">
			...
			<?php foreach ($a2_questions as $section => $questions): ?>
				...
				<?php foreach ($questions as $q): ?>
					<div class="mb-3 ms-3">
						<label><strong><?= htmlspecialchars($q['label']) ?></strong></label><br>
